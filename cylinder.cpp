...
void Cylinder(float x,float y,float z , float th,float ph , float R,float H)
{
   ...
   for (j=-1;j<=1;j+=2)
      for (i=0;i<N;i++)
      {
         double th0 = j* i   *360.0/N;
         double th1 = j*(i+1)*360.0/N;
   ...
   //  Cylinder Body (strip of quads)
      for (i=0;i<N;i++)
      {
         ...
         Point T[4] = { {       0,static_cast<float>(th0/90.0), 0} , {       2,static_cast<float>(th0/90.0), 0} , {       2,static_cast<float>(th1/90.0), 0} , {       0,static_cast<float>(th1/90.0), 0} };
         DrawPoly(P,N,T,4);
      }
